...
import com.threadly.post.like.response.PostLikersApiResponse.PostLiker;
...
public class PostLikeQueryService implements GetPostLikersUseCase {
  ...
  @Override
  public PostLikersApiResponse getPostLikers(GetPostLikersQuery query) {
    /*게시글 유효성 검증*/
    ...
    /*게시글 좋아요 목록 조회*/
    List<PostLiker> allLikerList = fetchPostLikePort.getPostLikersBeforeCreatedAt(
        query.getPostId(),
    ...
    return new PostLikersApiResponse(
        pagedLikerList,
        cursorLikedAt,
        cursorLikerId
    );
