...
@Slf4j
public class ProfileImageCommandCommandService implements MyProfileImageCommandUseCase {
...
//    imageAspectRatioValidator.validate(uploadImageToList);
    ...
    userProfileCommandPort.create(userProfileImage);
  ...
  public void updateProfileImage(String userId, String profileImageId) {
    ...
    if (profileImageId != null && !userProfileImageQueryPort.existsNotDeletedByUserProfileImageId(
        profileImageId)) {
      throw new UserProfileImageException(ErrorCode.USER_PROFILE_IMAGE_NOT_EXISTS);
    }

    /*기존 이미지 id 조회*/
    String previousProfileImageId = userProfileImageQueryPort.findConfirmedProfileImageIdByUserId(
        userId).orElse(null);
    ...
    if (previousProfileImageId != null) {
      userProfileCommandPort.updateStatusById(previousProfileImageId, ImageStatus.DELETED);
    }
    /*새로운 이미지 설정*/
    if (profileImageId != null) {
      userProfileCommandPort.updateStatusAndUserIdByImageId(profileImageId, userId,
