...
@TestPropertySource(properties = {"spring.batch.job.name=hardDeletePostImageJob"})
@DisplayName("DeleteJobConfig 배치 작업 테스트")
@TestMethodOrder(MethodOrderer.OrderAnnotation.class)
class DeleteJobConfigTest extends BaseBatchTest {
  ...
  void shouldDeletePostImagesWithDeletedStatus(@Autowired Job hardDeletePostImageJob)
      throws Exception {
    // given
    ...
    jobLauncherTestUtils.setJob(hardDeletePostImageJob);
    ...
    assertThat(remainingImages).hasSize(3); // CONFIRMED 상태만 남음
    assertThat(remainingDeletedImages).hasSize(0); // DELETED 상태는 모두 삭제됨
    assertThat(remainingConfirmedImages).hasSize(3); // CONFIRMED 상태는 유지
  }
  ...
  void shouldDeletePostImagesWithDeletedStatus_02(@Autowired Job hardDeletePostImageJob)
      throws Exception {
    // given
    /* 데이터 삽입*/
    int DELETE_STATUS_SIZE = 3;
  ...
  void shouldCompleteSuccessfullyWhenNoDeletedImages(@Autowired Job hardDeletePostImageJob)
      throws Exception {
    // given
  ...
  void shouldExecuteStepSuccessfully() throws Exception {
    ...
    // when
    JobExecution jobExecution = jobLauncherTestUtils.launchStep("hardDeletePostImageStep");
